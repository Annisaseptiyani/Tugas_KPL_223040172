       ...
       if ($error === 0) {
           if ($image_size > 130000) {
              ...
              if (in_array($image_ex, $allowed_exs )) {
                  ...
                  $sql = "INSERT INTO post(post_title,post_author,post_text,category, cover_url) VALUES (?,?,?,?,?)";
                  $stmt = $conn->prepare($sql);
                  $res = $stmt->execute([$title,$author, $text, $category, $new_image_name]);
              }else {
          ...
          $sql = "INSERT INTO post(post_title,post_author, post_text, category) VALUES (?,?,?,?)";
          $stmt = $conn->prepare($sql);
          $res = $stmt->execute([$title,$author, $text, $category]);
      }
